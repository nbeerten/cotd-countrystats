...
export async function getTokens(
    ...
    if(await kv.exists(UBISOFT_RATELIMIT_ENDSAT)) {
        ...
        if(endsAt > Date.now()) {
            throw new Error(`Ubisoft rate limit reached. Rate limit ends at ${new Date(endsAt).toLocaleString()}`);
        }
    }
    ...
    if(await kv.exists(UBISOFT_TICKET)) {
        ...
        if(ubisoftPreventRateLimit > Date.now()) {
            throw new Error(`Prevent-rate-limit system rate limit reached. Block ends at ${new Date(ubisoftPreventRateLimit).toLocaleString()}`);
        }
        ...
        if ('ticket' in ubisoftTicketJson !== true) {
            ...
            } else if('httpCode' in ubisoftTicketJson && (ubisoftTicketJson.httpCode as number) > 400) {
                await kv.set(UBISOFT_RATELIMIT_ENDSAT, Date.now() + 100 * 60 * 5, {
                    ex: 60 * 5 // 5 minutes
                });
            }
            throw new Error('Error while retrieving ticket from Ubisoft Services', { cause: ubisoftTicketJson });
        } else {
    ...
    if ('accessToken' in tokens !== true && 'refreshToken' in tokens !== true) {
        throw new Error('Error while retrieving accessToken from Nadeo Services', { cause: tokens });
    }
