...
export async function getFullCompetitionLeaderboard(NadeoClub: NadeoClubClient, competitionId: string | number, length = 10, offset = 0) {
    ...
    for (let i = 0; i < iterationsNeeded; i++) {
        ...
        const iterationPlayers = await NadeoClub.getCompetitionLeaderboard(competitionId, IDsLeft, offset + i * 255);
